...
const SOURCE_DIR = path.resolve(__dirname, '../../recipes_sources/fr');
...
async function importRecipes() {
    try {
        ...
        for (const file of files) {
            ...
            // e.g., .../fr/healthy/7-day.json -> category=healthy, subcategory=7-day
            ...
            for (const recipe of recipes) {
                ...
                // 1. Try to find existing recipe by specific URL or Image URL
                // We'll prioritize image_url as it's in the schema we recalled
                ...
                // 2. Insert/Update Translation (French)
                // Assuming 'fr' as language code

                // Check if translation exists
                const existingTrans = await client.execute({
                    sql: "SELECT id FROM recipe_translations WHERE recipe_id = ? AND language_code = 'fr'",
                    args: [recipeId]
                });
